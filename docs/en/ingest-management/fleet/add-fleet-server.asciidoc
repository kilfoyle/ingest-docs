...
ifeval::["{flag-serverless}"=="yes"]
IMPORTANT: On-premises {fleet-server} is not currently available for use with link:{serverless-docs}[{serverless-full}] projects. In a {serverless-short} environment we recommend using {fleet-server} hosted on {ess}.
endif::[]
